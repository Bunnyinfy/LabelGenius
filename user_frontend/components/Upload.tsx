...
export const Upload = () => {
  ...
  // Function to submit the task after payment
  ...
  // Function to handle the payment and call submission
  async function makePayment() {
    try {
      // Get a new blockhash before creating the transaction
      const {
        context: { slot: minContextSlot },
        value: { blockhash, lastValidBlockHeight },
      } = await connection.getLatestBlockhashAndContext();

      // Increase lastValidBlockHeight to give the transaction more time
      const adjustedBlockHeight = lastValidBlockHeight + 150; // Adds more tolerance

      const transaction = new Transaction().add(
        SystemProgram.transfer({
          fromPubkey: publicKey!,
          toPubkey: new PublicKey(
            // "Ca731bcvNF2srmKMXtnimmYjnwNH472gFLJwMd8mtNdu"
            "2vXR4a3VxPzMzLyoaF3BJLZKDHLb8L6iygswJeyk4bjv"
          ),
      ...
      // Confirm transaction within blockheight using adjusted block height
      await connection.confirmTransaction(
        {
          blockhash,
          lastValidBlockHeight: adjustedBlockHeight, // Use adjusted block height
          signature,
        },
        "confirmed" // Use 'confirmed' for faster confirmation
      );
